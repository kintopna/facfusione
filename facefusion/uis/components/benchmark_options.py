...
from facfusione import wording
...
def render() -> None:
	...
	BENCHMARK_RESOLUTIONS_CHECKBOX_GROUP = gradio.CheckboxGroup(
		...
		choices = facfusione.choices.benchmark_resolutions,
		value = facfusione.choices.benchmark_resolutions
	)
	BENCHMARK_CYCLE_COUNT_SLIDER = gradio.Slider(
		label = wording.get('uis.benchmark_cycle_count_slider'),
		value = 5,
		step = 1,
		minimum = min(facfusione.choices.benchmark_cycle_count_range),
		maximum = max(facfusione.choices.benchmark_cycle_count_range)
	)
